...
def main():
    ...
    # Find ONLY the Day images
    day_images = [f for f in os.listdir(s1_train) if f.startswith("Day_") and f.endswith(('.png', '.jpg'))]
    day_images.sort()

    # Pick 5% for validation
    val_count = int(len(day_images) * 0.05)
    random.seed(42) # Keeps it reproducible
    val_images = random.sample(day_images, val_count)

    print(f"Moving {val_count} Day images to validation folders...")
